...
import Thumbnail from './ImagesThumbnail';
...
module.exports = Field.create({
	...
	componentWillUpdate (nextProps) {
		const value = _.map(this.props.value, 'filename').join();
		const nextValue = _.map(nextProps.value, 'filename').join();
		if (value !== nextValue) {
			this.setState(this.buildInitialState(nextProps));
		}
	},
	buildInitialState (props) {
		const uploadFieldPath = `Images-${props.path}-${++uploadInc}`;
	...
	renderUI () {
		...
		return (
			<FormField label={label} className="field-type-images" htmlFor={path}>
				<div>
					{thumbnails}
				</div>
