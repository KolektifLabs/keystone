...
var debug = require('debug')('keystone:fields:image');
...
image.properName = 'Image';
util.inherits(image, FieldType);

/**
 * Registers the field on the List's Mongoose Schema.
 */
image.prototype.addToSchema = function (schema) {
 ...
 * Uploads a new file
 */
image.prototype.upload = function (item, file, callback) {
	var field = this;
	// TODO; Validate there is actuall a file to upload
	debug('[%s.%s] Uploading image for item %s:', this.list.key, this.path, item.id, file);
...
image.prototype.remove = function (item) {
 ...
 */
image.prototype.isModified = function (item) {
	var modified = false;
 ...
 */
image.prototype.validateInput = function (data, callback) {
 ...
 */
image.prototype.validateRequiredInput = function (item, data, callback) {
 ...
 */
image.prototype.updateItem = function (item, data, files, callback) {
	// Process arguments
